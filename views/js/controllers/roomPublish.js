'use strict';
angular.module( 'app' )
  .controller( 'roomPublishController', [ '$rootScope', '$scope', '$state', '$stateParams', '$filter', 'resource', '$uibModal', 'growl',
    function ( $rootScope, $scope, $state, $stateParams, $filter, resource, $uibModal, growl ) {
      ...
      resource.events.getByID( {
          id: $stateParams.id
        },
        function success( event ) {
          self.event = event;
          self.seed = self.event.seed;
          resource.rosters.getByID( {
              id: event.rosterID
            },
            function success( roster ) {
              self.roster = roster;
              resource.rooms.getByID( {
                  id: event.roomID
                },
                function success( room ) {
                  ...
                  self.countSeatsInIslands();
                  self.addNoteBelowEachIsland();
      ...
      self.findIslandsHelper = function ( i, j, island ) {
        ...
        var toPush = self.virtualMap[ i ][ j ].realCell;
        toPush.vY = i;
        toPush.vX = j;
        self.islands[ island ].push( toPush );
      ...
      /**
       * Removes the outside borders from the room display.
       * @return nonthing
       */
      self.removeOutsideBorders = function () {
        // Iterate through the columns of the room
      ...
      /**
       * Adds a nore for the expected seats and expected students below each
       * island
       * @return {[type]} [description]
       */
      self.addNoteBelowEachIsland = function () {
        self.islands.forEach( function ( island ) {
          var maxX = 0;
          var maxY = 0;
          var numSeated = 0;
          var numEmpty = 0;
          island.forEach( function ( seat ) {
            if ( seat.isEmpty ) {
              numEmpty++;
            } else {
              numSeated++;
          ...
          var infoCell = self.virtualMap[ self.virtualMap.length - 1 ][ maxX - 1];
          infoCell.isIslandInfo = true;
          infoCell.numSeated = numSeated;
          infoCell.numEmpty = numEmpty;
          console.log(infoCell);
        } );
      }

      /**
       * Counts the number of seats in each of the islands that exist within
       * the room.
       * @return The number of seats in each of the islands
       */
      self.countSeatsInIslands = function () {
        // log things for now
        var rowStarts = [];
        for ( var i = 0; i < self.virtualMap.length; i++ ) {
          var lastVx = 0;
          for ( var j = 0; j < self.virtualMap[ i ].length; j++ ) {
            var curCell = self.virtualMap[ i ][ j ];
            if ( curCell.realCell ) {
              if ( curCell.vX - lastVx > 1 && curCell.realCell.id != "" ) {
                rowStarts.push( curCell );
              }
              lastVx = curCell.vX;
            }
          }
        ...
        rowStarts.forEach( function ( rowStart ) {
          var startX = rowStart.vX;
          var startY = rowStart.vY;
          var curCell = self.virtualMap[ startY ][ startX ];
          var isEmptyCount = 0;
          var isSeatedCount = 0;
          while ( curCell.realCell ) {
            if ( curCell.realCell.isEmpty ) {
              isEmptyCount++;
            } else {
              isSeatedCount++;
            }
            curCell = self.virtualMap[ curCell.vY ][ curCell.vX + 1 ];
          }
          curCell.isRowInfoCell = true;
          curCell.isEmptyCount = isEmptyCount;
          curCell.isSeatedCount = isSeatedCount;
        } );
      }
