'use strict';
angular.module( 'app' )
  .controller( 'pdfDownloadController', [ '$rootScope', '$scope', '$state', '$stateParams', '$filter', 'resource', '$uibModalInstance', 'growl', 'students', 'event', 'room',
    function ( $rootScope, $scope, $state, $stateParams, $filter, resource, $uibModalInstance, growl, students, event, room ) {
      ...
      students = students.filter( function ( elmt ) {
        return elmt.email != null
      } );

      var generateHeader = function () {
        if ( self.predicate.val == "ln" ) {
          return {
            alignment: 'right',
            columns: [ {
                table: {
              ...
              {
                text: " Tutor Recording: _____________",
                style: ""
              }
            ]
          };

        } else if ( self.predicate.val == "row" ) {
          return {
            alignment: 'right',
            columns: [ {
                table: {
      ...
      self.generateDoc = function ( grid ) {
        ...
        return {
          content: [ generateHeader(), generateSubHeader(), generatePredicate(), metaData, {
            columns: []
          } ],
          styles: {
            forStaff: {
      ...
      self.download = function ( predicate ) {
        ...
        case 'ln':
          ...
          self.extra = ' FOR STUDENTS';
          self.extra2 = ' Tutor Recording: ___________';
          break;
        case 'lnmdt':
          container = students.sort( self.sortByName );
        ...
        if ( midterm ) {
          ...
          pdfMake.createPdf( self.docDefinition )
            .open();
          pdfMake.createPdf( self.docDefinition )
            .download( "StudentVersion" + self.pdfName + self.formatText );
          ...
          pdfMake.createPdf( self.docDefinition )
            .open();
          pdfMake.createPdf( self.docDefinition )
            .download( self.pdfName + self.formatText );
        } else {
      ...
      self.writeStudents = function ( docDefinition, container, colIndex, exclude ) {
        ...
        var totalStudents = "\n\nTotal Students: " + ( totalStudents + specialSeatingNum )
          .toString() + "\n";
        var totalSeats = "Total Seats: " + room.totalSeats.toString() + "\n";
        var actualPresent = "# of Students Absent: _____\n";
      ...
      self.writeGroupStudents = function ( docDefinition, containers, exclude ) {
        ...
        for ( var index = 0; index < containers.length; index++ ) {
          ...
          docDefinition.content[ colIndex ].columns.push( {
            text: rangeStr,
            bold: true,
            fontSize: 30
          } );
      ...
      self.toString = function ( student ) {
        ...
        if ( student.isOsd ) {
          special = " (Different Room)";
        } else if ( student.exclude ) {
          special = " (Special Seating)";
        }
      ...
      self.separateStudents = function ( container ) {
        ...
        var threshold = ( container.length / 4 + 1 );
